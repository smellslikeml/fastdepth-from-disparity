...
## MobileNetSkipAdd two inputs

class MobileNetSkipAddDisp(nn.Module):
    def __init__(self, output_size, pretrained=False):

        super(MobileNetSkipAddDisp, self).__init__()
        self.output_size = output_size
        mobilenet_rgb = imagenet.mobilenet.MobileNet()
        mobilenet_disp = imagenet.mobilenet.MobileNet()
        if pretrained:
            pretrained_path = os.path.join('mobilnet', 'model_best.pth.tar')
            ...
            for k, v in state_dict.items():
                name = k[7:] # remove `module.`
                new_state_dict[name] = v
            mobilenet_rgb.load_state_dict(new_state_dict)
            mobilenet_disp.load_state_dict(new_state_dict)
        else:
            mobilenet_rgb.apply(weights_init)
            mobilenet_disp.apply(weights_init)

        for i in range(14):
            setattr( self, 'conv{}_rgb'.format(i), mobilenet_rgb.model[i])
            setattr( self, 'conv{}_disp'.format(i), mobilenet_disp.model[i])

        kernel_size = 5
        self.decode_conv1_rgb = nn.Sequential(
            depthwise(1024, kernel_size),
            pointwise(1024, 512))
        self.decode_conv1_disp = nn.Sequential(
            depthwise(1024, kernel_size),
            pointwise(1024, 512))

        self.decode_conv2_rgb = nn.Sequential(
            depthwise(512, kernel_size),
            pointwise(512, 256))
        self.decode_conv2_disp = nn.Sequential(
            depthwise(512, kernel_size),
            pointwise(512, 256))

        self.decode_conv3_rgb = nn.Sequential(
            depthwise(256, kernel_size),
            pointwise(256, 128))
        self.decode_conv3_disp = nn.Sequential(
            depthwise(256, kernel_size),
            pointwise(256, 128))

        self.decode_conv4_rgb = nn.Sequential(
            depthwise(128, kernel_size),
            pointwise(128, 64))
        self.decode_conv4_disp = nn.Sequential(
            depthwise(128, kernel_size),
            pointwise(128, 64))

        self.decode_conv5_rgb = nn.Sequential(
            depthwise(64, kernel_size),
            pointwise(64, 32))
        self.decode_conv5_disp = nn.Sequential(
            depthwise(64, kernel_size),
            pointwise(64, 32))

        self.decode_conv6 = pointwise(32, 1)

        weights_init(self.decode_conv1_rgb)
        weights_init(self.decode_conv2_rgb)
        weights_init(self.decode_conv3_rgb)
        weights_init(self.decode_conv4_rgb)
        weights_init(self.decode_conv5_rgb)

        weights_init(self.decode_conv1_disp)
        weights_init(self.decode_conv2_disp)
        weights_init(self.decode_conv3_disp)
        weights_init(self.decode_conv4_disp)
        weights_init(self.decode_conv5_disp)
    ...
    def forward(self, x, y):
        ...
        if len(rgb.shape) < 4 or len(disp.shape) < 4:
            ...
            disp = disp[None, :]
        for i in range(14):
            layer_rgb = getattr(self, 'conv{}_rgb'.format(i))
            layer_disp = getattr(self, 'conv{}_disp'.format(i))
        ...
        for i in range(1,6):
            layer_rgb = getattr(self, 'decode_conv{}_rgb'.format(i))
            layer_disp = getattr(self, 'decode_conv{}_disp'.format(i))

            rgb = layer_rgb(rgb)
            disp = layer_disp(disp)

            rgb = F.interpolate(rgb, scale_factor=2, mode='nearest')
            disp = F.interpolate(disp, scale_factor=2, mode='nearest')
